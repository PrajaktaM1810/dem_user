...
getUserDetails({id}) async {
  ...
  try {
    ...
    print('ID: $id');
    print('URL: ${(ismulitipleride) ? "${url}api/v1/user?current_ride=$id" : "${url}api/v1/user"}');
    print('Headers: {Content-Type: application/json, Authorization: Bearer ${bearerToken[0].token}}');
...
etaRequestWithPromo({outstation}) async {
  ...
  try {
    ...
    print('Response: ${response.body}');  // Added print statement to show the response
...
rentalEta() async {
  dynamic result;
  try {
    print('API Method URL: ${url}api/v1/request/list-packages');

    print('Request URL: ${url}api/v1/request/list-packages');
...
createRequestLaterPromo() async {
  ...
  try {
    ...
    print("Request_Parameters: $requestBody");
...
// Bidding API = api/v1/request/create
